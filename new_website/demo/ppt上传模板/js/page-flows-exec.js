...
async function launchRun() {
    ...
    try {
        ...
        // Template: binary upload (only user-uploaded, not defaults)
        for (const f of pfTemplate) {
            if (!f.isDefault && f.binary) {
                uploads.push(
        ...
        // Content + style: text upload
        for (const f of [...pfContent, ...pfStyle]) {
