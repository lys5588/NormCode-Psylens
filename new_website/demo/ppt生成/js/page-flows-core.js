/**
 * Page-Flows Core — PPT Generation variant
 * Navigation, server connection, model picker, defaults loader.
 *
 * loadDefaults reads:
 *   [样式参考元数据] → pfTemplate (html_template) + pfStyle (guide)
 * (User may optionally upload .html templates which are merged with style guides.)
 */
...
async function connectToServer() {
    ...
    try {
        ...
        try {
            ...
            const preferred  = availableModels.find(m => m.id === 'qwen-turbo' || m.name === 'qwen-turbo-latest');
